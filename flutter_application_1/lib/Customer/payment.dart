...
class payment extends StatefulWidget {
  final String Username;
    const payment({required this.Username});
  
      @override
  paymentState createState() =>paymentState(Username: Username);
}
...
class paymentState extends State<payment> {
  final String Username;
    paymentState ({required this.Username});
                ...
                child: Row(
                  ...
                  children: [
                    Column(
                      children: [
                        CircleAvatar(
                          backgroundImage: NetworkImage('http://192.168.1.105/foryou_project/Profilepicture_caregiver/${list[index]['ProfilePicture']}'),
                          radius: 50,
                        ),
              ...
              Align(
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text("ประวัตืด้านการทำงาน",style: const TextStyle(
                                fontWeight: FontWeight.bold, fontSize: 20,color: Colors.amber),),
                      SizedBox(height: 2,),
                       Text("\t\tปัจจุบัน : "+list[index]['presentjob'],style: const TextStyle(
                                fontWeight: FontWeight.bold,fontSize: 16),),
                             
                       Padding(padding: const EdgeInsets.only(
      left: 30.0, right: 10.0, top: 3, bottom: 0),
                       child:Text(list[index]['Work'],style: const TextStyle(
                                fontSize: 15),)),
                                SizedBox(height: 2,),
                       Text("\t\tความสามารถเพิ่มเติม",style: const TextStyle(
                                fontWeight: FontWeight.bold,fontSize: 16),),
                                   Padding(padding: const EdgeInsets.only(
      left: 30.0, right: 10.0, top: 3, bottom: 0),
                       child:Text(list[index]['moreability'],style: const TextStyle(
                                fontSize: 15),)),

                                      Padding(padding: const EdgeInsets.only(
      left: 10.0, right: 10.0, top: 20, bottom: 0),
                       child:Text("เลือกวันในการจอง",style: const TextStyle(
                                fontWeight: FontWeight.bold,fontSize: 16),)),
                      SizedBox(height: 3,),

...
class logincaregiver extends StatefulWidget {
  static const routeName = '/logincaregiver';
  const logincaregiver({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return logincaregiverState();
  }
}

class logincaregiverState extends State<logincaregiver> {
  ...
  login(Username, Password) async {
    ...
    print(data.toString());
    final response = await http.post(Uri.parse(ApiUrl.logincaregiver),
        headers: {
    ...
    try {
      if (response.statusCode == 200) {
        ...
        if (!resposne['error']) {
          ...
          print(" User name ${user['CaregiveID']}");
          savePref(1, user['Username'], user['CaregiverID']);
  ...
  savePref(int value, String Username, int CaregiverID) async {
    SharedPreferences preferences = await SharedPreferences.getInstance();

    preferences.setInt("value", value);
    preferences.setString("Username", Username);
    preferences.setString("CaregiverID", CaregiverID.toString());
    preferences.commit();
  }
}
